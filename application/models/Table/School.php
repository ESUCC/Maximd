...
class Model_Table_School extends Model_Table_AbstractIepForm
{
 ...
 // This function was pulled from iepweb02 because the /student/edit/id_student/xx did not renber to the zend version
...
//    protected $_primary = array('id_county', 'id_district', 'id_school');
  ...
  /*
   * Mike uncommented this out 2-13-2017 in order to make the grop action Transfer Students work. For Some reason Maxim commented this out.
   * along with a lot of other things in this section.
   * 
   */
  function getCurrentManager($idCounty, $idDistrict, $idSchool)
    {
        $table = new Model_Table_School();
// PUt in by Mike to debug

    $myfl=fopen("/tmp/currentmanager.txt","w");
    fwrite($myfl,$idCounty);fwrite($myfl,$idDistrict);fwrite($myfl,$idSchool);
