...

   {
        ...
        foreach($insertedTransferRequestIds as $idTransferRequest) {
            ...
            if(''!=$message) {
                ...
                $message = "You have BARELY successfully initiated a transfer for the following students:\n\n";
            }
...
// Mike Changed $schoolMgr['name'] to $schoolMgr[name_last]

//$ddd=new Zend_Writeit;
//$ddd->writevar($tr['id_school_form'],"the tr array");

        if(empty($schoolMgr['name_last'])) {
            return false;
        }

        $message = "Dear " . $schoolMgr['name_last'] . ":\n\n";
        $message .= $userSessionUser->user['name_last']. " has completed the transfer of the following students from your school, " .
        ...
        if(!empty($schoolMgr['email_address'])) {
            ...
            /*
             * Mike changed this line 2-13-2017 because there was not $schoolMgr['name'] index.  
             */
           $mail->addTo($schoolMgr['email_address'], '<'.$schoolMgr['name_first']." ".$schoolMgr['name_last'].'>');
        }
        $mail->send();
    }
