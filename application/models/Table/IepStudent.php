...
class Model_Table_IepStudent extends Zend_Db_Table_Abstract {
    ...
    public function getTeamStudents($staffId,$id_School,$id_county,$id_district){
        ...
        $sql=('SELECT s.name_first,s.name_last,s.id_district,s.id_county,s.id_student,s.id_school,t.id_personnel,t.id_student,t.flag_edit,t.flag_view,t.flag_create
               from iep_student s,iep_student_team t where s.id_school=\''.$id_School.'\' 
               and s.id_district=\''.$id_district.'\' and s.id_county=\''.$id_county.'\' and 
               t.id_personnel=\''.$staffId.'\' and t.id_student=s.id_student and 
               ( t.status=\'Active\' or t.status=\'Removed\' or t.status=\'Inactive\') order by s.id_district,s.name_last');
