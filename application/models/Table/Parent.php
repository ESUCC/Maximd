...
//    protected $_primary = array('id_county', 'id_district', 'id_school');
    ...
    public function parentSave($options)
    {
		...
		else 
		{
	 		...
	 		$data = array( 'user_name'  => $options["user_name"] );
    ...
    public function parentDelete($options)
    {
        $db = Zend_Registry::get('db');
        $db->delete('iep_guardian', 'id_student = '.$options['id_student'].' and id_guardian = '.$options['id_guardian'] );
    }
