...
//  const PARENT = 3;
    ...
    // Think where to move it later
    const SA   = 1;    // system admin
    const DM   = 2;    // district manager
    const ADM  = 3;    // associate district manager
    const SM   = 4;    // school manager
    const ASM  = 5;    // associate school manager
    const CM   = 6;    // case manager
    const SS   = 7;    // school staff
    const SP   = 8;    // specialist
    const PG   = 9;    // parent/guardian
    const SC   = 10;   // service coordinator
    ...
    protected $sqlStmtArray = array();
    protected $fromStmt = null;
    protected $fromStmtArray = array();
    protected $whereStmt = null;
    protected $whereStmtArray = array();
    protected $orderLimitStmt = null;
    protected $orderLimitStmtArray = array();
...
//             $fromStmtArray = array('s' => 'iep_student');    
//         } else {
//             $fromStmt = " FROM my_students AS s ";
//             $fromStmtArray = array('s' => 'my_students');
//         }
//         /*
...
//             );
            ...
            $select = $this->getStudentSearchSelect($options, $sort);
            $paginator = new Zend_Paginator(new Zend_Paginator_Adapter_DbSelect($select));
            $paginator->setItemCountPerPage($options['maxRecs']);
            $paginator->setCurrentPageNumber(empty($options['page']) ? 1 : $options['page']);
            $count = $paginator->getTotalItemCount();
            $maxResultsExceeded = $count > 2500;
            return array(
                $maxResultsExceeded,
                $paginator,
                $key,
...
//        }
    ...
    private function getStudentSearchSelect($options, $sort) {
        $privileges = $this->_session->user->privs;

        $select = $this->_student->getAdapter()->select();

        $sqlStmtArray = array('student' => '*', 'name_case_mgr' => 'get_name_personnel(id_case_mgr)');
        ...
        switch ($options['format']) {
            ...
            default:
                ...
                $sqlStmtArray += array(
                    'mdt_primary_disability' => 'get_most_recent_mdt_disability_primary(student.id_student)',
                    'mdt_date_conference' => 'get_most_recent_mdt_date_conference(student.id_student)',
                    'det_notice_date' => 'get_most_recent_determination_notice(student.id_student)',
                    'mdt_id' => 'most_recent_final_mdt_id(student.id_student)',
                    'mdt_draft_id' => 'get_most_recent_mdt_draft_id(student.id_student)',
                    'draft_form_type' => 'rpt_draft_form_type(student.id_student)',
                    'draft_iep_date_created' => 'rpt_draft_date_created(student.id_student)',
                    'draft_iep_id' => 'rpt_draft_id(student.id_student)',
                    'form_type' => 'rpt_final_form_type(student.id_student)',
                    'iep_date_conference' => 'rpt_final_date_created(student.id_student)',
                    'iep_date_conference_duration' => 'rpt_final_date_created_duration(student.id_student)',
                    'iep_id' => 'rpt_final_id(student.id_student)',
                    'mdtorform001_draft_form_type' => 'mdtorform001_draft_form_type(student.id_student)',
                    'mdtorform001_draft_date_created' => 'mdtorform001_draft_date_created(student.id_student)',
                    'mdtorform001_draft_id' => 'mdtorform001_draft_id(student.id_student)',
                    'mdtorform001_final_form_type' => 'mdtorform001_final_form_type(student.id_student)',
                    'mdtorform001_final_date_created' => 'mdtorform001_final_date_created(student.id_student)',
                    'mdtorform001_final_id' => 'mdtorform001_final_id(student.id_student)'
                );
            break;
        }

        $sqlStmtArray += array(
            'name_county' => 'get_name_county(student.id_county)',
            'age' => new Zend_Db_Expr("date_part('year', age(dob))"),
            'dob' => 'dob',
            'name_district' => 'get_name_district(student.id_county, student.id_district)',
            'name_school' => 'get_name_school(student.id_county, student.id_district, student.id_school)',
            'name_full' => new Zend_Db_Expr("case when student.name_middle is not null then student.name_first || ' ' || student.name_last else student.name_first || ' ' || student.name_last end"),
            'name_last_first' => new Zend_Db_Expr("student.name_last || ', ' || student.name_first"),
            'address' => new Zend_Db_Expr("student.address_street1 || ', ' || student.address_city || ' ' || cast(student.address_state as text) || ', ' || cast(student.address_zip as text)")
        );

        $select->joinLeft(['team' => 'iep_student_team'], new Zend_Db_Expr("team.id_student = student.id_student and team.status::text = 'Active'::text"), array());

        /*
         * Add join for personnel based on search form
        */
        if (array_key_exists('case_load_first_name', $options['searchValues']) || array_key_exists('case_load_last_name', $options['searchValues'])
            || array_key_exists('team_member_first_name', $options['searchValues']) || array_key_exists('team_member_first_name', $options['searchValues'])) {
            $select->joinLeft(['p' => 'iep_personnel'], 'student.id_case_mgr = p.id_personnel or team.id_personnel = p.id_personnel', array());
        }

        $select->distinct()->from(array('student' => 'iep_student'), $sqlStmtArray);
        $whereCondtitions = array();

        if($this->_session->parent) {
            $id_guardian = $this->_session->id_guardian;
            $select->where("student.id_student IN (select id_student from iep_guardian WHERE id_guardian = $id_guardian)");
        } else {
            foreach ($privileges as $privilege) {
                
                $id_school = $privilege['id_school'];
                $id_district = $privilege['id_district'];
                $id_county = $privilege['id_county'];
                $id_personnel = $privilege['id_personnel'];

                switch ($privilege['class']) {
                    case (self::SA):            // 1 - system admin
                        // need dumb condition here to have OR statements populated correctly
                        $whereCondtitions []= " (1=1) ";
                    break;
                    case (self::DM):            // 2 - district manager
                    case (self::ADM):           // 3 - associate district manager
                        $whereCondtitions []=" (student.id_district = '$id_district' and student.id_county = '$id_county') ";
                        break;
                    case (self::SM):            // 4 - school manager
                    case (self::ASM):           // 5 - associate school manager
                        $whereCondtitions []= " (student.id_school = '$id_school' and student.id_district = '$id_district' and student.id_county = '$id_county') ";
                        break;
                    case (self::CM):            // 6 - case manager
                        $whereCondtitions []= " (student.id_school = '$id_school' and student.id_district = '$id_district' and student.id_county = '$id_county' and (student.id_case_mgr = '$id_personnel' or student.id_ei_case_mgr = '$id_personnel' or team.id_personnel = '$id_personnel')) ";
                        break;
                    case (self::SS):            // 7 - school staff
                        $whereCondtitions []= " (student.id_school = '$id_school' and student.id_district = '$id_district' and student.id_county = '$id_county' and team.id_personnel = '$id_personnel') ";
                        break;
                    case (self::SP):            // 8 - specialist
                        $whereCondtitions []= " (student.id_school = '$id_school' and student.id_district = '$id_district' and student.id_county = '$id_county' and (student.id_ser_cord = '$id_personnel' or student.id_ei_case_mgr = '$id_personnel' or team.id_personnel = '$id_personnel')) ";
                        break;
                    case (self::SC):            // 10 - service coordiantor
                        $whereCondtitions []= " (student.id_school = '$id_school' and student.id_district = '$id_district' and student.id_county = '$id_county' and (student.id_ser_cord = '$id_personnel' or team.id_personnel = '$id_personnel')) ";
                    break;
                }
            }

            $select->where("(" . implode('or', $whereCondtitions) . ")");
        }
        ...
        if (!empty($options['search_limit_caseload'])) {
            if (1 == $options['search_limit_caseload']) {
                $sessIdUser = $this->_session->sessIdUser;
                $select->where("(id_ei_case_mgr = '$sessIdUser' OR id_list_team ilike '%$sessIdUser%')");
            }
        }
         ...
         */
        if("reports" == $options['controller'] && "transportation" == $options['action'] && "Qualified for Transport" == $options['tranStatus']) {
            $select->where("'t' = (select transportation_yn from iep_form_004 where id_form_004 = (select most_recent_final_iep_id(student.id_student)))");
        } elseif("reports" == $options['controller'] && "transportation" == $options['action'] && "Did Not Qualify" == $options['tranStatus']) {
            $select->where("'f' = (select transportation_yn from iep_form_004 where id_form_004 = (select most_recent_final_iep_id(student.id_student)))");
        } elseif("student" == $options['controller'] && "search" == $options['action'] && "No NSSRS ID#" == $options['searchOther']) {
            $select->where("unique_id_state is null");
        } elseif("reports" == $options['controller'] && "nssrs" == $options['action'] && "No NSSRS ID#" ==$options['searchOther']) {
            $select->where("unique_id_state is null");
        } elseif("reports" == $options['controller'] && "nssrs_transfers" == $options['action'] && "No NSSRS ID#" == $options['searchOther']) {
            $select->where("unique_id_state is null");
        } elseif("reports" == $options['controller'] && "nssrs" == $options['action'] && "excluded" == $options['format']) {
            $select->where("exclude_from_nssrs_report = true");
        }
        ...
        if (isset($options['searchStatus']) && 'All' !== $options['searchStatus']) {
            ...
            $select->where("student.status = '$searchStatus'");
        }
        ...
        if (isset($options['searchFilter'])) {
            switch ($options['searchFilter']) {
                case 'Missing NSSRS#':
                    $select->where("student.unique_id_state IS NULL");
                    break;
                case 'CM Only':
                    $select->where("student.id_case_mgr IS NOT NULL");
                    break;
                case 'Team Member Only':
                    $select->where("team.id_student is not null");
                    break;
                case 'None':
                default:
                    break;
        ...
        if(false == $this->_session->parent) {
            ...
            foreach ($options['searchValues'] as $key => $value) {
                ...
                switch ($key) {
                    case 'pub_school_student':
                        if ($value == "T" || $value == "t" || $value == "true") {
                            $select->where("pub_school_student = 'TRUE'");
                        } elseif($value == "F" || $value == "f" || $value == "false") {
                            $select->where("pub_school_student = 'FALSE'");
                        }
                        break;
                    case 'onteam':
                        $whereStmt .= " $conditional ";
                        $binds[] = $value;
                        $select->where("team.id_personnel = '$value' and team.status = 'Active'");
                        break;
                    case 'primaryOrRelatedService':
                        $select->where("( id_student IN (SELECT id_student FROM iep_form_004 where lower(primary_disability_drop) = $value) or id_student IN (SELECT id_student FROM form_004_related_service where lower(related_service_drop) = $value) )");
                        break;
                    case 'isCM':
                        $select->where("id_case_mgr = $value");
                        break;
                    case 'isSC':
                        $select->where("id_ser_cord = $value");
                        break;
                    case 'isECIM':
                        $select->where("id_ei_case_mgr = $value");
                        break;
                    case 'id_student':
                    case 'ss.id_student':
                        if ( substr_count($value, ",") > 0 ) {
                            $select->where("id_student in ($value)");
                        }  else {
                            $select->where("id_student = $value");
                        }
                        break;
                    case 's.grade':
                        if ( substr_count($value, ",") > 0 ) {
                    ...
                    case 's.gradegreaterthan':
                        if('ei' == $value || 'ei 0-2' == $value) {
                            $binds[] = $value;
                            $select->where("(student.grade ilike 'ecse' or student.grade = 'K' or student.grade > $value)");
                        }  elseif('ecse' == $value) {
                            $select->where("(student.grade = 'K' or student.grade > $value)");
                        }  elseif('K' == $value) {
                            $select->where("(student.grade > $value)");
                        }  else {
                            $select->where("(grade_to_integer(student.grade) > grade_to_integer($value))");
                        }
                        break;
                    case 's.gradelessthan':
                        // Updating this conditional for SRSSUPP-388
                        if('ei' == $value || 'ei 0-2' == $value) {
                            $select->where("student.grade ilike 'ei 0-2'");
                        }  elseif('ecse' == $value) {
                            $select->where("(student.grade ilike 'ei 0-2')");
                        }  elseif('K' == $value) {
                            $select->where("(student.grade ilike 'ecse' or student.grade ilike 'ei 0-2')");
                        }  else {
                            $select->where("(grade_to_integer(student.grade) < grade_to_integer($value))");
                        }
                        break;
                    ...
                    case 'case_load_first_name':
                        $select->where("(p.name_first ilike $value)");
                        break;
                    case 'case_load_last_name':
                        $select->where("(p.name_last ilike $value)");
                        break;
                    case 'team_member_last_name':
                        $select->where("(p.name_last ilike $value)");
                        break;
                    ...
                    case 'ell_student':
                        if ($value == "T" || $value == "t" || $value == "true" || $value == "Y" || $value == "y" || $value == "Yes" || $value == "yes") {
                            $select->where("ell_student = 't'");
                        } elseif($value == "F" || $value == "f" || $value == "false" || $value == "N" || $value == "n" || $value == "No" || $value == "no") {
                            $select->where("ell_student = 'f'");
                        }
                        break;
                    case 's.alternate_assessment':
                        if ($value == "T" || $value == "t" || $value == "true" || $value == "Y" || $value == "y" || $value == "Yes" || $value == "yes") {
                            $select->where("alternate_assessment = 't'");
                        } elseif($value == "F" || $value == "f" || $value == "false" || $value == "N" || $value == "n" || $value == "No" || $value == "no") {
                            $select->where("alternate_assessment = 'f'");
                        }
                        break;
                    ...
                    default:
                         ...
                         */
                        if (is_numeric($value)) {
                            $select->where("$sf = $value");
                        } else {
                            $select->where("lower($sf) LIKE '$value%'");
                        }
                        break;
                }
            }
        ...
        if (!empty($sort)) {
            ...
            switch ($sort['field']) {
                case 'name_full':
                    $select->order(array("student.name_last $sortDirection", "student.name_first $sortDirection"));
                    break;
                case 'mdt':
                    $select->order(array("mdtorform001_final_date_created $sortDirection"));
                    break;
                case 'iep':
                    $select->order(array("rpt_date_sort(student.id_student) $sortDirection"));
                    break;
                case 'manager':
                    $select->order(array("name_case_mgr $sortDirection"));
                    break;
                default:
                    $select->order(array($sort['field'] . ' ' . $sort['direction']));
                    break;
            }
            $select->order(array("student.name_last"));
        } else {
            $select->order(array("student.name_last"));
        }
