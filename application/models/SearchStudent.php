...
//	const PARENT = 3;
	...
	{
	   // THis is the main function for listing students in the /student/search screen iepweb02.
	   // Mike 2-21-2017
	//  include("Writeit.php");  
	...
	// Mike changed this and it workded 3-3-2017 it was 200M
	    ini_set('memory_limit', '300M');
	    /*  writevar($options['showAll'],"this is the opions array line 57 search student.php\n"); // If not exist it makes it with nothing
	      writevar($options['defaultCacheResult'],"this is the cache result line 60\n");// If  not exist so it make it.  Usually a 1 in there
	      writevar($options,'tthis is the options');*/
	    /*
	...
	//    writevar($options['searchValue'], "this is the search value line 65 mdl \n");
        ...
        if($this->_session->parent) {
          ...
          //  writevar($admin,'this is the $admin'); // will return false for ordinary user.
          // Mike tried this 2-20-2017
             // $admin= 3 >= $privCheck->getMinPriv()?true:false;
            //  writevar($admin,'this is the admin');
...
// Mike changed this 3-16-2016: Figure 200 should be good enough
     ...
     //   writevar($sqlStmt,'this is the sql statement');
        $t=$this->_student->getAdapter()->fetchAll($sqlStmt . $fromStmt . $whereStmt . $orderLimitStmt, $binds);
     //   writevar($t,'this is the list of students');
