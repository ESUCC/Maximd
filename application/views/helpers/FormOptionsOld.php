...
//        $student = $myStudentsObj->find($session->sessIdUser, $id_student);
        ...
        if (isset($formAccessArr[$form->status]['view']) && $formAccessArr[$form->status]['view']) {
        	$options[] = '<a href="https://iepdev.nebraskacloud.org/srs.php?area=student&sub=form_'.$form->form_no.'&document='.$form->id.'&page=1&option=view">View</a>';
        }
        if('Draft'==$form->status) {
            if (isset($formAccessArr[$form->status]['edit']) && $formAccessArr[$form->status]['edit']) {
                $options[] = '<a href="https://iepdev.nebraskacloud.org/srs.php?area=student&sub=form_'.$form->form_no.'&document='.$form->id.'&page=1&option=edit">Edit</a>';
            }
            if (isset($formAccessArr[$form->status]['delete']) && $formAccessArr[$form->status]['delete']) {
                $options[] = '<a href="https://iepdev.nebraskacloud.org/srs.php?area=student&sub=form_'.$form->form_no.'&document='.$form->id.'&page=1&option=delete">Delete</a>';
            }
            if (isset($formAccessArr[$form->status]['finalize']) && $formAccessArr[$form->status]['finalize']) {
                $options[] = '<a href="https://iepdev.nebraskacloud.org/srs.php?area=student&sub=form_'.$form->form_no.'&document='.$form->id.'&page=1&option=finalize">Finalize</a>';
            }
        }
        if('Suspended'==$form->status) {
            ...
            if (isset($formAccessArr['Draft']['edit']) && $formAccessArr['Draft']['edit']) {
                $options[] = '<a href="https://iepdev.nebraskacloud.org/srs.php?area=student&sub=form_'.$form->form_no.'&document='.$form->id.'&page=1&option=resume">Resume Draft Status</a>';
            }
        }
        if (isset($formAccessArr[$form->status]['log']) && $formAccessArr[$form->status]['log']) {
            $options[] = '<a href="https://iepdev.nebraskacloud.org/srs.php?area=student&sub=form_'.$form->form_no.'&document='.$form->id.'&page=1&option=log">Log</a>';
        }
        if (isset($formAccessArr[$form->status]['print']) && $formAccessArr[$form->status]['print']) {
            $options[] = '<a href="https://iepdev.nebraskacloud.org/form_print.php?form=form_'.$form->form_no.'&document='.$form->id.'">Print</a>';
        }
