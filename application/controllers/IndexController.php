...
class IndexController extends App_Zend_Controller_Action_Abstract
{
    ...
    public function indexAction()
    {
    	...
    	if('production' == APPLICATION_ENV) {
    		$this->_redirect('https://iep.unl.eduu/');
    	}
    	$this->restoreMessage();
...
//        echo 'frank1';die();
    ...
    public function loginredirectAction() {
		...
		if('' != $token) {
			...
			// this is the session record for the iep.unl.eduu site
...
//	    		}
	    		...
	    		return $this->_redirect('https://iep.unl.eduu/srs.php?area=personnel&sub=gettoken&destination='.$this->getRequest()->getParam('destination'));
	    	} else {
        ...
        if($this->getRequest()->getParam('destination')) {
    		...
    		$this->_redirect('https://iep.unl.eduu/srs.php?area=home&sub=home&option=1');
    	} else {
    		$this->_redirect('/home');
    	} 
