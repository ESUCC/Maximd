...
// Maxim modified this code October 20 2016
    ...
    public function parentAction()
    {
        ...
        header("Location:https://iepd.nebraskacloud.org/srs.php?area=student&sub=student&student=".$request->id_student."&option=parents");
        exit;
    }
	...
	public function saveAction()
    {
   	...
   	// ----   Read all Privileges ------------------------
		...
		foreach ($post as $nameField => $valueField) {
			...
			if (	strlen($options["name_first"]) != "" &&
				strlen($options["name_last"]) != "" &&
				...
				//($validator_date->isValid($options["sesis_exit_date"]) || $options["sesis_exit_date"] == "") 
				 ...
				 if (isset($options["ward_surrogate"])) $options["ward_surrogate"] = ($options["ward_surrogate"] == "Yes") ? 'True' : 'False'; else $options["ward_surrogate"] = "False";
				 if (isset($options["ward_surrogate_nn"])) $options["ward_surrogate_nn"] = ($options["ward_surrogate_nn"] == "Yes") ? 'True' : 'False'; else $options["ward_surrogate_nn"] = "False";

				 if (!isset($options["date_web_notify"]) || !$validator_date->isValid($options["date_web_notify"])) $options["date_web_notify"] = NULL;
				 $options["unique_id_state"] = intval($options["unique_id_state"] * 1);

				$studentQuery = new Model_Table_StudentFormAdd();
				$id_student = $studentQuery->studentSave($options);  // Save data

				// Get Guardian/Parent
				if (intval($options["unique_id_state"]) > 0) {
    				    $url = "https://" . $_SERVER["HTTP_HOST"] . "/Newedfi/studentParents?student_id=".$options["unique_id_state"];
				    $result = file_get_contents($url);
				    $result = json_decode($result);

				    if (count($result) > 0 ) {
				        $studentQuery = new Model_Table_StudentFormAdd();
    					$result = $studentQuery->studentParentAdd($id_student, $options, $result);  // Save data
				    }
				}

				header("Location: /student/edit/id_student/".$id_student);
				exit;

		 	} else {
			   $this->view->msg = "Incorrect data"; //print_r($options);
		 	}
...
//        $this->_helper->layout()->disableLayout();
    ...
    public function chartsAction()
    {
        ...
        header("Location:https://iepd.nebraskacloud.org/srs.php?area=student&sub=student&student=".$request->id_student."&option=charting");
        exit;
...
//    }
    ...
    public function searchAction() {
       ...
       // $this->writevar1($this->getRequest()->getParam('pref_student_search_location'),'this is the preferred search location');
...
//    }
    ...
    public function editAction()
    {
     // include("Writeit.php");
        ...
        /* This was added by Mike 6-21-2017 so that we could we could make edfi updates
           when users saved the the student edit screen.  See additional line 1138
           
        */
    
         $edfiStudentData=$postData;
            if($edfiStudentData!=null){
                 
                if($edfiStudentData['status']=='Active'){
                    $edfiStudentData['sesis_exit_code']=null;
                    $edfiStudentData['sesis_exit_date']=null;
                }
            
            
                $distInfo=new Model_Table_Edfi();
                $distInfo->updateEditStudentEdfi($edfiStudentData);
            }    
        ...
        if($this->getRequest()->getParam('id_student')) {
            ...
            if(1!=count($studentArr)) {
             ...
             //  $this->writevar1($dbStudent,'this is the student updated line 1012');
           ...
           $this->view->id_student = $this->getRequest()->getParam('id_student');
            ...
            $url = Zend_Controller_Request_Http::SCHEME_HTTPS . "://iepd.nebraskacloud.org/srs.php?area=student&sub=student&student=".$dbStudent['id_student']."&option=edit";
        ...
        if($this->getRequest()->getParam('viewOnly')) {
            ...
            if($formValid) {
                // valid to save
                $data = $form->getValues();
              //  writevar($data,'this is the data line 1088');
        ...
        if(count($form->getMessages())) {
            ...
            if($this->getRequest()->isPost()) {
            //    $this->writevar1($stuData,'this is the student data inside the post line 1150');
...
//            App_Vlog::file($config->job_queue_root.'/jobqueue/group-print/print-list-of-forms.php');
    ...
    public function studentadminAction() {   }

    public function studentmergeAction() {
	$student_id_A = $this->getRequest()->getParam('student_id_A');
	$student_id_B = $this->getRequest()->getParam('student_id_B');
	$studentsList = new Model_Table_StudentSearch();
	$result = $studentsList->studentsList($student_id_A, $student_id_B);
        $jsonData = array ( 'studentsList' => $result ); 
	$this->_helper->json->sendJson($jsonData);
    }

    public function studentmergesaveAction() {

	$student_id_A = $this->getRequest()->getParam('student_id_A');
	$student_id_B = $this->getRequest()->getParam('student_id_B');
	$cm_merge = $this->getRequest()->getParam('cm_merge');

	//print ">". $student_id_A . "|" .$student_id_B . "|" . $cm_merge;

	$studentsMerge = new Model_Table_StudentSearch();
	$result = $studentsMerge->studentsMerge($student_id_A, $student_id_B, $cm_merge);

        $jsonData = array ( 'studentsMergeRes' => $result ); 
	$this->_helper->json->sendJson($jsonData);
    }
