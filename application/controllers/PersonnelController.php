...
class PersonnelController extends My_Form_AbstractFormController
{
    ...
    public function viewAction()
    {
     ...
     //   writevar($id_personnel,'this is the id of the personnel');
    
        $personnelObj = new Model_Table_PersonnelTable();
        $privilegesObj = new Model_Table_IepPrivileges();
        
        $this->view->privileges = $privilegesObj->getPrivileges($id_personnel);
     /*   writevar($this->view->privileges,'this is the view privileges of the user');
      * This brings back an array of each school from the privilege table keyed on id_personnel
      * 
     */  
        foreach($this->view->privileges as $key => $priv) {
          /* the key is the index on the array from above and the priv is 
           * obviously, the fields
           * 
            
            writevar($key,'this is the key');
            writevar($priv,'this is the priv');
           ...
           /*
            * writevar($this->usersession->sessIdUser,'this is the usersessionid');// this only get the userid from the session variables 
           writevar($this->view->privileges[$key]['access'],'this is the access going to the view');
           All arrays are coming back false and still not rendering the view even 
           if we switched this to $this->view->privileges[$key]['access']='true'; to test.  
