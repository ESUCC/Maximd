...
class ParentController extends My_Form_AbstractFormController
{
	...
	foreach($priv as $priv_array)
        ...
        if ($priv_student == 1) echo $this->view->render('parent/edit.phtml');
		    else echo $this->view->render('parent/access-denied.phtml');
	...
	foreach($priv as $priv_array)
        ...
        if ($priv_student == 1) echo $this->view->render('parent/delete.phtml');
		    else echo $this->view->render('parent/access-denied.phtml');
    ...
    public function deletedoneAction()
    {
        ...
        $parentQuery->parentDelete($options); // Parent Delete result

        $session = new Zend_Session_Namespace('searchParentForm'); // Remove Parent/Guardian from Cache search result list
	if (isset($session->searchKey) && Model_CacheManager::isCached(Zend_Registry::get('searchCache'), $session->searchKey)) Model_CacheManager::removeCache(Zend_Registry::get('searchCache'), $session->searchKey);
