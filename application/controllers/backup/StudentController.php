...
class StudentController extends My_Form_AbstractFormController
{
    ...
    public function logAction()
    {
      ...
      header("Location:https://iepd.nebraskacloud.org/srs.php?area=student&sub=student&student=".$request->id_student."&option=log");
        exit;
...
//            header("Location:$DOC_ROOT/srs.php?area=student&sub=student&student=".$post->goto_id_student."&option=charting");
    ...
    function formsAction() {
     ...
     //   writevar($this->view->student,'this is the student view');
...
//    }
    ...
    public function searchAction() {
        ...
        $this->setupStudentSearch(); // This is in /library/app/Zend/controller/Action/Abstract.php
    }
    ...
    public function searchStudentAction() {
      ...
      //  writevar( $this->view->page,'this is the page param'); which returns a 1
        for($i=0;$i<6;$i++) {
        ...
        /*
          ...
          */
        $sort = array();
      // writevar($formatFields,'this is format fields'); Just puts out an array of identifiers like headings
        foreach ($formatFields AS $field) {
            if (!empty($field)) {
               //writevar($field,'this is the field'); an array of headings: id_student,nuame_full,name_district,name_county,name_school,role
       ...
       //  writevar($field,'this is after everything in the field formatfield as'); Not sure what this does because same as line 325
        }
      ...
      // $t = $search->searchStudent($this->getRequest()->getParams(), $sort);
      //  writevar($t,'this is the view->field');
        if (!array_key_exists('error', ($searchResults = $search->searchStudent($this->getRequest()->getParams(), $sort)))) {
           ...
           // writevar($this->view->defaultCacheResult,'the result cache'); return the number 1 as string
            $viewScript = 'student/search-student-table.phtml';
        } else {
       ...
       writevar($t,'thi sis the jason encode part'); 
       this looks like it is trying to got cache and retrieve it rom cache with a url.  See /logs/textfile.txt in the home directory
 */
      // writevar($viewScript,'this is the viewScript var'); which returns "student/search-student-table.phtml"
    ...
    public function printResultsAction() {
      ...
      //  writevar($t,'this is to see if we got there');
        $this->_helper->layout()->disableLayout();
        $this->_helper->viewRenderer->setNoRender(true);
...
//    }
    ...
    public function searchStudentFormsAction() {
     ...
     //   writevar($params,'this is params in SearchStudent controller');
        if (!array_key_exists('error', ($searchResults = $search->searchForms($params, $sort)))) {
         ...
         //   writevar($session,'this is the session stuff');
            $studentObj = new Model_Table_StudentTable();
            $student = $studentObj->studentInfo($this->getRequest()->getParam('student'));
            $this->view->student = $student[0];
          //  writevar($student[0],'thi is the student');
        } else {
            $this->view->error = $searchResults['error'];
