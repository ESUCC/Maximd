...
class LoginController extends App_Zend_Controller_Action_Abstract
{
    ...
    public function indexAction()
    {
     ...
     // writevar($request,'this is the request line 23 \n');
        if ($request->isPost()) {
            if ($this->view->userLogonForm->isValid($this->getRequest()->getPost())) {
               // writevar($this->view->userLogonForm,'this is the userlogonform');// It most certainly is and passes username and password along
             //  writevar($this->_forward('dologin'),'this is the do login');// returned null
    ...
    public function dologinAction()
    {
      ...
      //  writevar($userName,'this should be the username');
      // writevar($password,'this should be the password');
        
        
        
        $this->auth = new App_Auth_Authenticator();
        $user = $this->auth->getCredentials($userName, $password);
     //   writevar($user,'this is the credentials');
        ...
        if ($user) {
            ...
            // Mike added this  2-27-2017  it is where you want 
            // to put the check for update password should it be reset by sysadmin  2-22-2017
            $idPersonnel=$_SESSION['user']['id_personnel'];
            $staffMember= new Model_Table_IepPersonnel();
            $StaffInfo=$staffMember->getIepPersonnel($idPersonnel);
            
             $passwordFlag=$StaffInfo['password_reset_flag'];
          //   writevar($passwordFlag,'this is the pw reset flag');
            
             
             if($passwordFlag=='true'){
                 $StaffInfo['password_reset_flag']='false';
                 writevar($StaffInfo,'this is the staff info');
                 $staffMember->updateIepPersonnelPassReset($StaffInfo);
                 $this->redirect('Passwordchange/subpassword');
               // this will redirect outta the Passwordchange controller.   
             }
      
            // End of Where check the password_reset_flag in iep_personnel
    ...
    public function logoutAction()
    {
        ...
        if ('production' == APPLICATION_ENV) {
            $this->_redirect('http://iepd.nebraskacloud.org/');
        } else {
...
//        ));
    ...
    public function emailAccountSupervisors($personnelObj, $userFormData, $destinationSchoolsForThisUser=array(), $destinationDistrict, $newPersonnelId)
    {
        ...
        if (!empty($accountSupervisor['email_address'])) {
            ...
            $message .= " https://iepd.nebraskacloud.org/goto.php?goto=area&personnel=sub&personnel=personnel&$newPersonnelId=option&edit\n\n";
