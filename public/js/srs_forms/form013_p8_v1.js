...
function setupPage() {
    ...
    $('#team_members_parent').find('.tm_role').each(function (index, element) {
        showHideOtherOnRole(element);
    })
    $('#team_members_parent').find('.tm_role').on('change', function (event) {
        console.debug(event);
        showHideOtherOnRole(event.target);
    })
...
function showHideOtherOnRole(element) {
    if('Other' == $(element).val()) {
        $(element).closest('td').find('.tm_role_other').show();
    } else {
        $(element).closest('td').find('.tm_role_other').hide();
    }
}

function addSubFormAdditional(row) {
    $('#team_members_parent').last().find('.tm_role').each(function (index, element) {
        showHideOtherOnRole(element);
    });
    $('#team_members_parent').find('.tm_role').last().on('change', function (event) {
        console.debug(event);
        showHideOtherOnRole(event.target);
    });
}