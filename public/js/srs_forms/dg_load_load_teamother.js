var load_teamother = function() {
    //console.debug('running load_teamother');

    //var secondDlg = teammemberDialog();
    var formID = dojo.byId("id_form_004").value;
    
    var mainPrefix = "teamother_grid";
    var dialogName = "teamother_grid_dialog";
    var divToReplace = "replaceme_grid_iep_teamother"; // place div on layout (<div id="replaceme_grid_iep_teamother"></div>)

    var tableName = "iep_team_other";
    var tableKey = "id_iep_team_other";

    var serverStoreGetUrl = "/ajax/jsongetteamother/id/"+formID;
    var serverStoreUpdateUrl = "/ajax/jsonupdateteamother";
    
    var updateFields = ['participant_name', 'relationship_desc', 'relationship_other'];
    
    var addButtonId = "addrow_others";
    //
    ...
    // get data from remote server
    //
    teamother_grid_connObj = new ItemFileManagedStore(store, tableKey, updateFields, serverStoreUpdateUrl, mainPrefix);
    connectionObjects.push(teamother_grid_connObj);
    ...
    var makeDeleteButton = function (pk){
    	...
    	delButton = delButton + " onClick=\"teamother_grid_connObj.deleterow('"+pk+"')\"></div>";
    	return delButton;
    }
...
//    dojo.connect(addButtonId, "onclick", 'teamother_grid_connObj.addrow');

    //dojo.connect(dojo.byId(mainPrefix),'onclick',gridClicked);

    teamother_grid_connObj.init_database_connect();
