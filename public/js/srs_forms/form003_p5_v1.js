...
function setupPage() {
	...
	dojo.query('.myrecords_edit_link').forEach(
		function(node, index, arr){
            var callback = dojo.hitch( dialogWidget, dialogWidget.myFormsEdit, 'iep_form_003_returninfo', i);
	...
	dojo.query('.myrecords_add_link').forEach(
		function(node, index, arr){
            var callback = dojo.hitch( dialogWidget, dialogWidget.myFormsAdd, 'iep_form_003_returninfo', i);
            this.click = dojo.connect( node, "onclick", dialogWidget, callback );
            i = i+1;
		}
	);
