...
function save(event, wait2finish, url) {
    ...
    try {
		...
		$.blockUI({ 
			message: 'Your page better be saving...',
		});
...
//	dijit.byId('savingDialog').show();
//	dijit.byId('savingDialog').closeButtonNode.style.display='none'; 
...
//    console.debug('saving waittofinish:', wait2finish);
    var xhrArgs = {
...
//    startTimer();
    ...
    var pagesValidSpan = dojo.byId('pagesValid');
//	dojox.html.set(dojo.byId("pagesValid"), returneditems[0]['pageValidationList']);    
	dojox.html.set(dojo.byId("pagesValidTop"), returneditems[0]['pageValidationList']);    
	dojox.html.set(dojo.byId("pagesValid"), returneditems[0]['pageValidationList']);    
//console.debug('pagesValidSpan', returneditems[0]['pageValidationList']);
...
//	console.debug('validationPostSave');
    ...
    var valListSpan = dojo.byId('validationList');
//    valListSpan.innerHTML = '';
    dojox.html.set(dojo.byId("validationList"), '');
    

    valListSpan.appendChild(makeValidationListTable(errorCount, returneditems[0]['validationArr']));
    ...
    //
    var valSpan = dojo.byId('validationStatusSpan');
//    valSpan.innerHTML = '';
    dojox.html.set(dojo.byId("validationStatusSpan"), '');
    valSpan.appendChild(makeValidationStatusTable(errorCount));
...
{	
	...
	//
	//console.group('function:: colorInvalidFields')
//    console.debug('errorCount: ', errorCount);
//    console.debug('errorArray: ', errorArray);
    ...
    if(0 == errorCount)
    {
	    ...
	    dojo.forEach(errorArray, // loop through returned errors
	        function(valRow) {
//    		console.debug('field: ', valRow['field']);
//    		console.debug('wrapper: ', valRow['wrapper']);
	    		colorMeRed(valRow['field'], valRow['wrapper']);
//	    		dojo.addClass(getContainerCell(evt.target), "errored");
	    	}
	    );
    }
}
...
//        		console.debug('addrow - tabContainerFound', tabContainerFound);
        		if(tabContainerFound)
				{	
					// build and insert a content pane
//        			console.debug('subformName tabcontainer found', subformName);
                    if (subformName == 'ifsp_goals')
                        var tabTitle = 'Goal ';
                    else if (subformName == 'iep_form_004_goal')
                        var tabTitle = 'Goal ';
					...
					// append stylesheets
					addEditorStyleSheets(subformName, tabNum);
//					console.debug('addEditorStyleSheets', subformName, tabNum);
					tab4.startup(); 
...
function updateFadingOtherTextboxes() {
	try {
    	//alert("Trying to update other checkboxes status");
    	setOtherFieldsDisplay();
    	connectOtherCheckboxes();
    } catch(err) {
    	//alert("Couldn't call the methods");
    }
}
...
//                        save(evt, false, "/form"+form_num+"/jsonupdateiep");                    
    			...
    			if('View Student' == studentOptions.value) {
    				window.location.href='https://iepd.nebraskacloud.org/srs.php?area=student&sub=student&student='+dojo.byId('id_student').value+'&option=view';
    				
    			} else if ('Edit Student' == studentOptions.value) {
    				window.location.href='https://iepd.nebraskacloud.org/srs.php?area=student&sub=student&student='+dojo.byId('id_student').value+'&option=edit';
    				
    			} else if ('Student Charting' == studentOptions.value) {
    				window.location.href='https://iepd.nebraskacloud.org/srs.php?area=student&sub=student&student='+dojo.byId('id_student').value+'&option=charting';
    				
    			} else if ('Parent/Guardians' == studentOptions.value) {
    				window.location.href='https://iepd.nebraskacloud.org/srs.php?area=student&sub=student&student='+dojo.byId('id_student').value+'&option=parents';
    				
    			} else if ('Student Team' == studentOptions.value) {
    				window.location.href='https://iepd.nebraskacloud.org/srs.php?area=student&sub=student&student='+dojo.byId('id_student').value+'&option=team';
    				
    			} else if ('Student Forms' == studentOptions.value) {
    				window.location.href='https://iepd.nebraskacloud.org/srs.php?area=student&sub=student&student='+dojo.byId('id_student').value+'&option=forms';
    				
    			} else if ('Student Log' == studentOptions.value) {
    				window.location.href='https://iepd.nebraskacloud.org/srs.php?area=student&sub=student&student='+dojo.byId('id_student').value+'&option=log';
    			}
...
function updateEditorCallback(data)
{
	...
	console.debug('editorID', editorID, editorID.substring(editorID.length-7));
...
//	console.debug('getCheckedValue - radioName', radioName);
//	console.debug('getCheckedValue - containerNode', containerNode);
	var retValue = "";
	dojo.query('input:checked', containerNode).forEach(
		function(childNode) {
//			console.debug('childNode', childNode.id, childNode.name, childNode.value);
			if(radioName == childNode.name) {
//				console.debug('match', radioName == childNode.name, childNode.value);
				retValue = childNode.value;
				return childNode.value;
			}
		});
//	console.debug('retValue', retValue);
	return retValue;
}

function editorHistory(fieldName) {
	//http://localhost:10094/editor-history/index/formnum/2/id/12/field/mdt_00603e2b
	//http://localhost:10094/editor-history/index/formnum/2/id/13/field/mdt_00603e2b
	var formNum = dojo.byId('form_number').value;
...
//	console.debug('id_form_'+formNum, dojo.byId('id_form_'+formNum).value);
//	console.debug('linkString', linkString);
...
//	console.debug('editor', $('#'+editorId), $('#'+editorId).val());
	$('#'+editorId).val(value);
//	console.debug('editor iframe', $('#'+editorId+'-Editor_iframe').contents().find('body').html());
	$('#'+editorId+'_iframe').contents().find('body').html(value);
	return true;
}
