...
if(!dojo._hasResource["soliant.editor.plugins.SrsNormalizeStyle"]){ //_hasResource checks added by build. Do not use _hasResource directly in your code.
dojo._hasResource["soliant.editor.plugins.SrsNormalizeStyle"] = true;
dojo.provide("soliant.editor.plugins.SrsNormalizeStyle");

dojo.require("dijit._editor._Plugin");
dojo.require("dijit._editor.html");

dojo.experimental("soliant.editor.plugins.SrsNormalizeStyle");

dojo.declare("soliant.editor.plugins.SrsNormalizeStyle",dijit._editor._Plugin,{
	//	summary:
	//		This plugin provides SrsNormalizeStyle cabability to the editor.  It is
	...
	setEditor: function(editor){
		...
		// attach out additional filter
//		this.editor.contentDomPreFilters.push(dojo.hitch(this, this._postValidatedFilter));
...
//	_postValidatedFilter: function(node){
//		console.debug('_postValidatedFilter');
////		dojo.byId('debugger').value += "___inserthtmlImpl----------------------------------\r\n" + html;
//		
//		html = dojo.attr(node, "innerHTML");
//		
//		console.debug('pre-html', html);
//		// Apply all the filters to remove MS specific injected text.
////		var i;
////		for(i = 0; i < this._filters.length; i++){
////			var filter  = this._filters[i];
////			html = html.replace(filter.regexp, filter.handler);
////		}
//
//		var s = html;    
...
//		div.innerHTML = s;
//
//		console.debug('post-html', s);
//		
//		return node;
//	},
	
	_convertToSemantic: function(node){
		console.debug('_convertToSemantic');
		// summary:
	...
	_replaceParagraphsWithDivs: function(text) {
		var s = text;    
	    s = s.replace(/<p>(&nbsp;|\u00A0| )*<\/p>/gim, "<div></div>");
	    s = s.replace(/<p>/gim, "<div>");
	    s = s.replace(/<\p>/gim, "<\div>");
	...
	_replaceEmptySpansWithSpanBr: function(text) {
		var s = text;    
	    s = s.replace(/<span><\/span>/gim, "<span><br/></span>");//(&nbsp;|\u00A0| )*
	    return s;
	},
...
//			div.innerHTML = "";
//			
//			div = this._postValidatedFilter(div);
//			html = dijit._editor.getChildrenHtml(div);
//			div.innerHTML = "";
...
dojo.subscribe(dijit._scopeName + ".Editor.getPlugin",null,function(o){
	...
	if(name === "srsnormalizestyle"){
		o.plugin = new soliant.editor.plugins.SrsNormalizeStyle({
