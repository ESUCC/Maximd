...
class App_Helper_Session extends Zend_Acl 
{
	...
	static public function grantArchiverSiteAccess($user, $parent) 
	{
        ...
        $oldSiteClient = new Zend_Http_Client('https://iepd.nebraskacloud.org/logon.php?option=1', $httpParams);
        ...
        if($dom->query('#ferpa')->count()>=1) {
            // login failed
            Zend_Debug::dump('login failed on iepd.nebraskacloud.org');
            return false;
        } else {
            Zend_Debug::dump('login SUCCESS to iepd.nebraskacloud.org');
        }
