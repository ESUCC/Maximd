...
class App_Report_Nssrs
{
    ...
    {
       // $this->writevar1($forceTransferData,'this is the force transfer data');
        ...
        // This calls forms 2,4,13,22 and 23 latest iep
...
//        }
    ...
    public function buildReportData()
    {
        ...
        $data['field23'] = $this->buildAlternateAssessment($this->student->grade);
        ...
        $this->writevar1($data,'this is the data for the student');
        return $data;
    }
    ...
    { // params are getDate arrays
        ...
        if ($date_to['mday'] < $date_from['mday']) {
            $age['modMonthDays'] = Model_Table_StudentTable::days_in_month($date_to['mon'] - 1,
    ...
    function buildAlternateAssessment($grade)
    {
        $aa = $this->student->alternate_assessment;
        if ($grade > 2 && ('t' == $aa || 1 == $aa)) {
            return 1;
        } elseif ('f' == $aa || 0 == $aa) {
            return 2;
        }
    ...
    function buildPlacementType()
    {
        ...
        if($pub) {
            return 0;
        } elseif(!$pub && $parentalPlacement == 0) {
            return 2;
        } elseif(!$pub && $parentalPlacement == 1) {
            return 3;
        } elseif(!$pub && $parentalPlacement == 2) {
            return 4;
        }
    }
            ...
            if (count($serviceArr) > 0) {
                foreach ($serviceArr as $serviceRow) {
                    if (!empty($serviceRow['service_service'])) {
                        $serviceRow['service_service'] = strtolower($serviceRow['service_service']);
                    ...
                    if (!empty($serviceRow['service_where'])) {
                        $serviceRow['service_where'] = strtolower($serviceRow['service_where']);
    ...
    private function limitByTime($formNumber, $status = 'final', $dateField = 'date_notice', $time = 'today - 1 year')
    {
        ...
        if (!is_null($time) && !is_null($dateString) && strtotime($time) > strtotime($dateString)) {
            ...
            if(isset($this->expiredForms[$formNumber])) {
                $this->expiredForms[$formNumber] = true;
            }
        }
    }
