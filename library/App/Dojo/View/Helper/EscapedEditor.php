 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class App_Dojo_View_Helper_EscapedEditor extends Zend_Dojo_View_Helper_Dijit
{
    /**
     ...
     * @return string
     */
    public function escapedEditor($id, $value = null, $params = array(), $attribs = array())
    {
...
//            'value' => $value,
        ...
        // 
        if(isset($params['history_link']) && $params['history_link']) {
            
        	$html .= '<div class="editor-header">';
            	$html .= '<div class="editor-history-link">';
            	    $html .= '<a href="javascript:void(0);" onclick="editorHistory(\''.$id.'\');">Editor History</a> ';
            	$html .= '</div>';
            	$html .= '<div class="editor-message">';
            	    $html .= '';
            	$html .= '</div>';
        	$html .= '</div>';
        }
...
try {
	if(dojo.byId('$hiddenId')) {
		/* this code is generated dynamically in _createEditorOnSubmit in the EscapedEditor view helper App_Dojo_View_Helper_EscapedEditor */
	...
	console.debug('error in _createEditorOnSubmit');
}
...
try {
	zend.findParentForm = function(elementNode) {
		while (!elementNode.nodeName || elementNode.nodeName.toLowerCase() != 'form') {
			elementNode = elementNode.parentNode;
		}
		return elementNode;
	};
} catch (error) {
	console.debug('error in _createGetParentFormFunction');
}
EOJ;
