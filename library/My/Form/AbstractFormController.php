	...
	protected function buildSrsForm($document, $page, $raw = false) {
		...
		//if (0 == $this->view->db_form_data ['version_number']) {
			if ($this->getRequest()->getActionName() == 'print') {
				$this->_redirect('https://iepdev.nebraskacloud.org/form_print.php?form=form_'.$this->getFormNumber().'&document='.$document);
			} else {
				$this->_redirect('https://iepdev.nebraskacloud.org/srs.php?area=student&sub=form_' . $this->getFormNumber () . '&document=' . $document . '&option='.$this->getRequest()->getActionName());
			}
			die ();
		}
...
//		}
		...
		if (null !== $prevMdt) {
			if (0 <= $prevMdt ['version_number']) {
				...
				$sessQuickLinks->primary_disability = "<a href=\"https://iepdev.nebraskacloud.org/srs.php?area=student&sub=form_002&document={$prevMdt['id_form_002']}&page=1&option=view\" target=\"_blank\">{$form->getElement('disability_primary')->getMultiOption($prevMdt['disability_primary'])}</a>";
		...
		if (null !== $prevIep) {
			if (9 <= $prevIep ['version_number']) {
				...
				$sessQuickLinks->primary_service = "<a href=\"https://iepdev.nebraskacloud.org/srs.php?area=student&sub=form_004&document={$prevIep['id_form_004']}&page=1&option=view\" target=\"_blank\">{$prevIep['primary_disability_drop']}</a>";
		...
		if (null !== $prevIep) {
			if (9 <= $prevIep ['version_number']) {
				...
				$sessQuickLinks->prev_iep = "<a href=\"https://iepdev.nebraskacloud.org/srs.php?area=student&sub=form_004&document={$prevIep['id_form_004']}&page=1&option=view\" target=\"_blank\">{$prevIep['date_conference']}</a>";
		...
		if (null !== $prevForm003) {
			if (9 <= $prevForm003 ['version_number']) {
				...
				$sessQuickLinks->prev_notice_iep = "<a href=\"https://iepdev.nebraskacloud.org/srs.php?area=student&sub=form_003&document={$prevForm003['id_form_003']}&page=1&option=view\" target=\"_blank\">{$prevForm003['date_notice']}</a>";
		...
		if (null !== $prevForm010) {
			if (9 <= $prevForm010 ['version_number']) {
				...
				$sessQuickLinks->prev_pr = "<a href=\"https://iepdev.nebraskacloud.org/srs.php?area=student&sub=form_010&document={$prevForm010['id_form_010']}&page=1&option=view\" target=\"_blank\">{$prevForm010['date_notice']}</a>";
		...
		if (null !== $prevMdt) {
			if (9 <= $prevMdt ['version_number']) {
				...
				$sessQuickLinks->prev_mdt = "<a href=\"https://iepdev.nebraskacloud.org/srs.php?area=student&sub=form_002&document={$prevMdt['id_form_002']}&page=1&option=view\" target=\"_blank\">{$prevMdt['date_mdt']}</a>";
		...
		if (null !== $prevForm013) {
			if (9 <= $prevForm013 ['version_number']) {
				...
				$sessQuickLinks->prev_ifsp = "<a href=\"https://iepdev.nebraskacloud.org/srs.php?area=student&sub=form_013&document={$prevForm013['id_form_013']}&page=1&option=view\" target=\"_blank\">{$prevForm013['meeting_date']}</a>";
		...
		if (null !== $prevForm001) {
			if (9 <= $prevForm001 ['version_number']) {
				...
				$sessQuickLinks->last_initial_eval = "<a href=\"https://iepdev.nebraskacloud.org/srs.php?area=student&sub=form_001&document={$prevForm001['id_form_001']}&page=1&option=view\" target=\"_blank\">{$prevForm001['date_notice']}</a>";
		...
		if (null !== $prevForm015) {
			if (9 <= $prevForm015 ['version_number']) {
				...
				$sessQuickLinks->last_initial_ifsp_eval = "<a href=\"https://iepdev.nebraskacloud.org/srs.php?area=student&sub=form_015&document={$prevForm015['id_form_015']}&page=1&option=view\" target=\"_blank\">{$prevForm015['date_notice']}</a>";
		...
		if (null !== $prevForm016) {
			if (9 <= $prevForm016 ['version_number']) {
				...
				$sessQuickLinks->last_initial_ifsp_placement = "<a href=\"https://iepdev.nebraskacloud.org/srs.php?area=student&sub=form_016&document={$prevForm016['id_form_016']}&page=1&option=view\" target=\"_blank\">{$prevForm016['date_notice']}</a>";
		...
		if (null !== $prevForm014) {
			if (9 <= $prevForm014 ['version_number']) {
				...
				$sessQuickLinks->last_notice_of_ifsp = "<a href=\"https://iepdev.nebraskacloud.org/srs.php?area=student&sub=form_014&document={$prevForm014['id_form_014']}&page=1&option=view\" target=\"_blank\">{$prevForm014['date_notice']}</a>";
	...
	function replaceSessionImages($url) {
		...
		return '<img class="form010Chart" src="http://iepweb02dev.nebraskacloud.org/temp/' . substr ( $url [1], strrpos ( $url [1], '/' ) + 1 ) . '.png">';
	}
...
//		$request = $this->getRequest ();
     ...
     //   $this->writevar1($document,'this is the document'); //Just print out the document number
       ...
       $view->assign("student_header", $this->view->studentInfoHeader($this->view->db_form_data, $this->getFormNumber()));
        ...
        $img = 'http://iepdev.nebraskacloud.org/image_upload/' . $this->view->db_form_data ['id_county'] . $this->view->db_form_data ['id_district'] . '.jpg';
...
//        }
	...
	public function deleteAction() {
		...
		if (isset ( $request->confirm ) && $request->confirm == "Confirm") {
			...
			if ('iepweb03' == APPLICATION_ENV) {
			  ...
			  $this->_redirect ( 'https://iepdev.nebraskacloud.org/srs.php?area=student&sub=student&student=' . $formData ['id_student'] . '&option=forms' );
			}
			return;
		}
	...
	public function createAction() {
		...
		if (!$studentObj->isDemoStudent($student[0]['id_county'],$student[0]['id_district'],$student[0]['id_school']) && in_array($this->getFormNumber(), $createOldForms['forms'])) {
			$this->_redirect('https://iepdev.nebraskacloud.org/srs.php?area=student&sub=form_' . $this->getFormNumber () . '&student=' . $this->getRequest()->student . '&option=new');
			exit;
		}
...
//		Zend_Debug::dump();die();
	...
	function doneAction() {
		...
		if ('iepweb03' == APPLICATION_ENV) {
		    ...
		    $this->redirect('https://iepweb02dev.nebraskacloud.org/student/search-forms/id_student/'.$current['id_student']);
		//	$this->_redirect ( 'https://iepdev.nebraskacloud.org/srs.php?area=student&sub=student&student=' . $current ['id_student'] . '&option=forms' );
		}
...
// Mike found this 2-23-2018 and it is the log  view to the old when get action is log.
		...
		if (9 > $dbData ['version_number']) {
			if ($this->getRequest()->getActionName() == 'print') {
		 	$this->_redirect('https://iepdev.nebraskacloud.org/form_print.php?form=form_'.$this->getFormNumber().'&document='.$this->getRequest ()->getParam ( 'document' ));
			} else {
		//	$this->_redirect('https://iepweb02dev.nebraskacloud.org/form'.$this->getFormNumber().'/'.$this->getRequest()->getActionName().'/document/'.$this->getRequest()->getParam('document'));
			$this->_redirect ( 'https://iepdev.nebraskacloud.org/srs.php?area=student&sub=form_' . $this->getFormNumber () . '&document=' . $this->getRequest ()->getParam ( 'document' ) . '&option='.$this->getRequest()->getActionName());
			}
			die ();
		}
