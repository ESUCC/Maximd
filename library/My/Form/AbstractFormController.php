...
abstract class My_Form_AbstractFormController extends App_Zend_Controller_Action_Abstract {  
	...
	public function preDispatch() {
		...
		if (! App_Helper_Session::siteAccessGranted ()) {
			if ('production' == APPLICATION_ENV) {
				...
				return $this->_redirect ( 'https://iepd.nebraskacloud.org/srs.php?area=personnel&sub=gettoken&destination=' . str_replace ( '/', '-', $_SERVER ['REQUEST_URI'] ) );
			} else {
				// redirect home
        ...
        // force password change if password_encourage_reset flag is on
        $passwordEncourageReset = isset($this->user->user['password_encourage_reset']) ? $this->user->user['password_encourage_reset'] : false;
        if ($passwordEncourageReset) {
            // message the user
            $this->view->message = '<div class="divNotification">Login successful. It is suggested that you <a href="https://iepd.nebraskacloud.org/srs.php?area=home&sub=password">reset your password</a>.</div>';
            $this->user->user['password_encourage_reset'] = null;
            // clear display flags
            $personnelObj = new Model_Table_PersonnelTable();
            $personnel = $personnelObj->find($this->user->user['id_personnel'])->current();
            $personnel->password_encourage_reset = null;
            $personnel->save();
        }
	...
	//	$this->writevar($modelform,'the modelform varaible in my/abst  form ctrl');
		...
		//$this->writevar1($this->view->db_form_data,'this is the data in the controller');
		...
		// redirect there if version is not greater or equal to 9
		 if (9 > $this->view->db_form_data ['version_number']) {
			if ($this->getRequest()->getActionName() == 'print') {
				$this->_redirect('https://iepd.nebraskacloud.org/form_print.php?form=form_'.$this->getFormNumber().'&document='.$document);
			} else {
				$this->_redirect('https://iepd.nebraskacloud.org/srs.php?area=student&sub=form_' . $this->getFormNumber () . '&document=' . $document . '&option='.$this->getRequest()->getActionName());
			}
			die ();
		}
		...
		$this->writeit1($this->view->db_form_data,'thi sis the form data');
	...
	public function finalizeAction() {
		...
		$t=$request->document;
		//$this->writevar1($t,'this is the request->document line 409');
		
		$this->view->page = $request->page;
		
		$tt=$request->page;
		//$this->writevar1($tt,'this is the request page line 414');
		
		$this->view->formNum = $this->getFormNumber ();
		
		$ttt=$this->getFormNumber();
		//$this->writevar1($ttt,'this is the form number line 419');
		
		
		$modelName = $this->getModelName ();
		
		
		/*$this->writevar1($modelName,'this is the model name line 423');
		  This will say Model_Form_004
		*/
		$modelobj = new $modelName ( $this->getFormNumber (), $this->usersession );
     	//$this->writevar1($modelobj,'this is the model object line 425');
		
		
		$formData = $modelobj->find ( $request->document );
		//$this->writevar1($formData,'this is the form data  line 430');
		...
		if (isset ( $request->confirm ) && $this->view->form_valid && $request->confirm == "Confirm") {
		    ...
		    /*
		     * Mike added 6-12-2017 so that we can publish to edfi table
		     * 
		     */
		  //  $this->writevar1($request->document,'this is the form data  line 430');
	...
	public function unfinalizeAction() {
		...
		/*
		 * Mike did this 6-14-2017 so that Wade could unfinalize forms.
...
//		}
		...
		if (null !== $prevMdt) {
			if (9 <= $prevMdt ['version_number']) {
				...
				$sessQuickLinks->primary_disability = "<a href=\"https://iepd.nebraskacloud.org/srs.php?area=student&sub=form_002&document={$prevMdt['id_form_002']}&page=1&option=view\" target=\"_blank\">{$form->getElement('disability_primary')->getMultiOption($prevMdt['disability_primary'])}</a>";
		...
		if (null !== $prevIep) {
			if (9 <= $prevIep ['version_number']) {
				...
				$sessQuickLinks->primary_service = "<a href=\"https://iepd.nebraskacloud.org/srs.php?area=student&sub=form_004&document={$prevIep['id_form_004']}&page=1&option=view\" target=\"_blank\">{$prevIep['primary_disability_drop']}</a>";
		...
		if (null !== $prevIep) {
			if (9 <= $prevIep ['version_number']) {
				...
				$sessQuickLinks->prev_iep = "<a href=\"https://iepd.nebraskacloud.org/srs.php?area=student&sub=form_004&document={$prevIep['id_form_004']}&page=1&option=view\" target=\"_blank\">{$prevIep['date_conference']}</a>";
		...
		if (null !== $prevForm003) {
			if (9 <= $prevForm003 ['version_number']) {
				...
				$sessQuickLinks->prev_notice_iep = "<a href=\"https://iepd.nebraskacloud.org/srs.php?area=student&sub=form_003&document={$prevForm003['id_form_003']}&page=1&option=view\" target=\"_blank\">{$prevForm003['date_notice']}</a>";
		...
		if (null !== $prevForm010) {
			if (9 <= $prevForm010 ['version_number']) {
				...
				$sessQuickLinks->prev_pr = "<a href=\"https://iepd.nebraskacloud.org/srs.php?area=student&sub=form_010&document={$prevForm010['id_form_010']}&page=1&option=view\" target=\"_blank\">{$prevForm010['date_notice']}</a>";
		...
		if (null !== $prevMdt) {
			if (9 <= $prevMdt ['version_number']) {
				...
				$sessQuickLinks->prev_mdt = "<a href=\"https://iepd.nebraskacloud.org/srs.php?area=student&sub=form_002&document={$prevMdt['id_form_002']}&page=1&option=view\" target=\"_blank\">{$prevMdt['date_mdt']}</a>";
		...
		if (null !== $prevForm013) {
			if (9 <= $prevForm013 ['version_number']) {
				...
				$sessQuickLinks->prev_ifsp = "<a href=\"https://iepd.nebraskacloud.org/srs.php?area=student&sub=form_013&document={$prevForm013['id_form_013']}&page=1&option=view\" target=\"_blank\">{$prevForm013['meeting_date']}</a>";
		...
		if (null !== $prevForm001) {
			if (9 <= $prevForm001 ['version_number']) {
				...
				$sessQuickLinks->last_initial_eval = "<a href=\"https://iepd.nebraskacloud.org/srs.php?area=student&sub=form_001&document={$prevForm001['id_form_001']}&page=1&option=view\" target=\"_blank\">{$prevForm001['date_notice']}</a>";
		...
		if (null !== $prevForm015) {
			if (9 <= $prevForm015 ['version_number']) {
				...
				$sessQuickLinks->last_initial_ifsp_eval = "<a href=\"https://iepd.nebraskacloud.org/srs.php?area=student&sub=form_015&document={$prevForm015['id_form_015']}&page=1&option=view\" target=\"_blank\">{$prevForm015['date_notice']}</a>";
		...
		if (null !== $prevForm016) {
			if (9 <= $prevForm016 ['version_number']) {
				...
				$sessQuickLinks->last_initial_ifsp_placement = "<a href=\"https://iepd.nebraskacloud.org/srs.php?area=student&sub=form_016&document={$prevForm016['id_form_016']}&page=1&option=view\" target=\"_blank\">{$prevForm016['date_notice']}</a>";
		...
		if (null !== $prevForm014) {
			if (9 <= $prevForm014 ['version_number']) {
				...
				$sessQuickLinks->last_notice_of_ifsp = "<a href=\"https://iepd.nebraskacloud.org/srs.php?area=student&sub=form_014&document={$prevForm014['id_form_014']}&page=1&option=view\" target=\"_blank\">{$prevForm014['date_notice']}</a>";
	...
	function replaceSessionImages($url) {
		...
		return '<img class="form010Chart" src="http://iepweb03.esucc.org/temp/' . substr ( $url [1], strrpos ( $url [1], '/' ) + 1 ) . '.png">';
	}
...
//		$request = $this->getRequest ();
        ...
        */
        ...
        $img = 'http://iepd.nebraskacloud.org/image_upload/' . $this->view->db_form_data ['id_county'] . $this->view->db_form_data ['id_district'] . '.jpg';
...
//        }
        ...
        if ($this->removeFile($tmpfpath) && $this->removeFile($tmpPDFpath) && file_put_contents(
            ...
            if ($princeResult) {
                ...
                */
      //         preg_replace_callback('/<img class="form010Chart" src="(.+?)">/',array(&$this, "deleteLocalImages"),$viewRendering);
	...
	public function deleteAction() {
		...
		$modelName = $this->getModelName ();
		$modelobj = new $modelName ( $this->getFormNumber (), $this->usersession );
	//	$this->writevar1($modelobj,'this is the model object');
		$formData = $modelobj->find ( $request->document );
		
		//$this->writevar1($formData,'this is the form data');
	    
	    /* Mike added this 4-18-2017  so that we can delete it anytime we want
	     * 
	    */
	   
	    
		if ('Final' == $formData ['status'] && 
		    'Admin' != $this->getAccess ( $formData ['id_student'], $this->usersession )->description) {
			
		    /* Mike added this 4-18-2017  jira SRS-42 so that we can delete it anytime we want
		     * the else is the additional part so that other forms can error out that
		     * are finalized
		     */
		...
		if (isset ( $request->confirm ) && $request->confirm == "Confirm") {
			...
			// Mike had to change this 3-14-2017 in order to have it go back to iepweb03 student search
			// screen otherwise it went to /student/forms/student/id of the student | this is 
			// a list of the forms to creaate of list.
			
			if ('iepweb03' == APPLICATION_ENV) {
			  // $this->redirectory-gotoSimple  ( 'forms', 'student', null, array ('student' => $formData['id_student'] ) );
				$this->_redirector->gotoSimple ( 'search-forms', 'student', null, array ('id_student' => $formData['id_student'] ) );
			} elseif ('jesselocal' == APPLICATION_ENV) {
				$this->_redirector->gotoSimple ( 'forms', 'student', null, array ('student' => $formData['id_student'] ) );
			} else {
			//$this->_redirect ( 'https://iepd.nebraskacloud.org/srs.php?area=student&sub=student&student=' . $formData ['id_student'] . '&option=forms' );
			$this->redirect('https://iepweb03.esucc.org/student/search-forms/id_student/'.$formData['id_student']);
			}
			return;
		}
	...
	public function createAction() {
		...
		//$this->writevar1($student,'the student in abstractformcontroller');
		...
		if (!$studentObj->isDemoStudent($student[0]['id_county'],$student[0]['id_district'],$student[0]['id_school']) && in_array($this->getFormNumber(), $createOldForms['forms'])) {
			$this->_redirect('https://iepd.nebraskacloud.org/srs.php?area=student&sub=form_' . $this->getFormNumber () . '&student=' . $this->getRequest()->student . '&option=new');
			exit;
		}
...
//		Zend_Debug::dump();die();
	...
	public function buildModel($document, $mode) {
		...
		$modelName = $this->getModelName ();
         //$this->writevar1($modelName,'this is model name'); // writes something like Model_Form018)
		
		
		// including subform data (related table rows)
		// also including student data
		$modelform = new $modelName ( $this->getFormNumber (), $this->usersession );
		//$this->writevar1($modelform,'this is the form ');// interesting that this does nULL
		$dbData = $modelform->find ( $document, $mode, 'all', null, true );
    //
        // Mike start here on Wednesday.  3-8-2017
		//$this->writevar1($dbData[0]['message'],'this is the dbData2');
		
		//$msg=$dbData[0]['message'];
		...
		/*This has all the data in it. only if it is true above.  That true is being passed from 
		 * AbstractFrom.php from public function buildDbForm.  If nobody is editing a certain form then
		 * then $dbdata will have the data. Otherwise it will return false.
		*/
		...
		//$tt=$modelform->formAccessObj->access_level;
		//writevar($tt,'this is access level');// this always seems to come back with editaccess
		...
		$ttt=$modelform->formAccessObj->description;
	//	writevar($ttt,'this is the model access object desc line 1931');  //form018 came back Admin
		/*
		 * construct the menu of student options (view student, edit student, etc)
		 */
		$this->view->accessArrayObj = $modelform->buildAccessObj ();
	//	writevar($this->view->accessArrayObj,'this is the accessArrayobjecf'); an array of access rules for each form.
	
		$dbData ['student_options'] = $this->studentOptionsMenu ( $this->view->accessArrayObj );
		//writevar($dbData,'this is the student options 1940');
		// Note the version number number is 1 and it wont work but 9 it does. 
		
		// this builds the pull down menu
		//$dbData['student_options'] = $this->buildStudentOptionsMenu();
		

		// get the version number
	//	writevar($dbData['version_number'],'this is the version number');
		...
		if (9 > $dbData ['version_number']) {
			...
			if ($this->getRequest()->getActionName() == 'print') {
				$this->_redirect('https://iepd.nebraskacloud.org/form_print.php?form=form_'.$this->getFormNumber().'&document='.$this->getRequest ()->getParam ( 'document' ));
			} else {
			    
			    // Mike did this June 21 to get the new form 
			    //writevar( $this->getFormNumber(),'this is the form  number');
			    if ($this->getFormNumber() != "018"){
				$this->_redirect ( 'https://iepd.nebraskacloud.org/srs.php?area=student&sub=form_' . $this->getFormNumber () . '&document=' . $this->getRequest ()->getParam ( 'document' ) . '&option='.$this->getRequest()->getActionName());
			   }
			    }
			die ();
		}
	...
	private function studentOptionsMenu($accessArrayObj) {
		...
		if ($accessArrayObj->accessArray ['team'] ['access']) {
			$access [] = 'Student Team'; // <--- && $actionName != "team"
		}
		if ($accessArrayObj->accessArray ['forms'] ['access']) {
			$access [] = 'Student Forms';
		}
...
//			}
	...
	public function editAction()
    {
		...
		$headtitle=( ' - ' . $this->getFormTitle () . ' Page ' . $this->view->page );

		
   	      
		/*This is the contents
		this is the form title
          string(39) " - Individualized Education Plan Page 1"
	   */
		...
		$dat=$this->buildModel ( $this->getRequest ()->getParam ( 'document' ), $this->view->mode );
...
//		}
	...
	//	writevar($config,'this is the config');
		...
		// get requested page, if any				   
		$this->view->page = ($this->getRequest ()->getParam ( 'page' ) > 0) ? $this->getRequest ()->getParam ( 'page' ) : $this->startPage;
		//writevar($this->view->page,'this is the view page x');
	// It always seems to return a number 1.  
	
		// set form title
		$this->view->headTitle ( ' - ' . $this->getFormTitle () . ' Page ' . $this->view->page );
	//	writevar($this->view->headTitle ( ' - ' . $this->getFormTitle () . ' Page ' . $this->view->page ),'this is headtitle');
		//all seems to be printed no matter what.
		
	//	writevar($t,'this is t');
		// build the model
		$this->view->db_form_data = $this->buildModel ( $this->getRequest ()->getParam ( 'document' ), $this->view->mode );
	   //  writevar($this->view->mode,'this is the view mode');
	   //  writevar($this->view->db_form_data,'this is the view db form data');
		// none of these have data in them for form018 unless it is somebody in deom district like Zend Man.
		...
		// build zend form
		$this->view->form = $this->buildZendForm ( $this->getFormClass (), $this->view->db_form_data, $this->view->version, $config, $this->view->page );
      // writevar($this->view->form,'thi sis the view form');
		// writevar($this->getFormClass(),'this is the function get form class');
		...
		//writevar($this->view->valid,'Is this a valid form line 2267');
