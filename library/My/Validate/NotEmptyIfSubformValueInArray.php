 ...
 * @author   Jesse LaVere
 */
class My_Validate_NotEmptyIfSubformValueInArray extends Zend_Validate_Abstract {
  ...
  public function doValid($validationFieldValue, $rowDataArray = null) {
    ...
    if(isset($rowDataArray[$subformPrefix]['count'])) {
	    for($x=1; $x<= $rowDataArray[$subformPrefix]['count']; $x++)
	    {
		    ...
		    if (is_array($valueThree)) {
		        if (in_array($subformEleVal, $valueThree)) {
		            $requireValue = true;
		        }
		    }
	    }
    ...
    if ($requireValue && is_null($validationFieldValue)) {
