 ...
 * @author   Jeremy Kendall 
 */
class My_Validate_NotEmptyIf extends Zend_Validate_Abstract {
...
//      Zend_Debug::dump($contextKey, $testValue);
...
//      Zend_debug::dump($contextKey, 'top');
//      Zend_debug::dump($testValue);
//      Zend_debug::dump($value);

    if ((null === $context) || !is_array($context) || !array_key_exists($contextKey, $context)) {
//        Zend_debug::dump('fail 0');
        $this->_error(self::KEY_NOT_FOUND);
        return false;
    }
 
    if (is_array($context[$contextKey])) {
        $parentFieldValue = $context[$contextKey][0];
    } else {
        $parentFieldValue = $context[$contextKey];
    }
//      Zend_debug::dump($parentFieldValue);
//      Zend_debug::dump($testValue == ($parentFieldValue));
//      Zend_debug::dump(empty($value));

//    Zend_debug::dump(is_null($testValue));
    if (!is_null($testValue)) {
    	if ($testValue == ($parentFieldValue) && empty($value)) {
//    		Zend_debug::dump('fail 1');
        	$this->_error(self::KEY_IS_EMPTY);
        	return false;
      	}
    } else {
      	if (!empty($parentFieldValue) && empty($value)) {
//            Zend_debug::dump('fail 2');
        	$this->_error(self::KEY_IS_EMPTY);
        	return false;
      	}
    }
