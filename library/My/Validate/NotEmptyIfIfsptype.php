 ...
 * @author   Jeremy Kendall 
 */
class My_Validate_NotEmptyIfIfsptype extends Zend_Validate_Abstract {
  ...
  public function doValid($value, $context = null) {
    ...
    //print("<pre>");var_dump($context);exit();
...
//    Zend_debug::dump(is_null($testValue));
    if (!is_null($testValue)) {
    	if ($testValue == ($parentField) && empty($value) && $context['ifsptype'] == 'Periodic') {
//    		Zend_debug::dump('here');
        	$this->_error(self::KEY_IS_EMPTY);
        	return false;
      	}
    } else {
      	if (!empty($parentField) && empty($value) && $context['ifsptype'] == 'Periodic') {
        	$this->_error(self::KEY_IS_EMPTY);
        	return false;
      	}
    }
