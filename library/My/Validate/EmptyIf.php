 ...
 * @author   Jeremy Kendall 
 */
class My_Validate_EmptyIf extends Zend_Validate_Abstract {
   ...
   */
  const KEY_NOT_EMPTY   = 'keyNotEmpty';
  ...
  protected $_messageTemplates = array(
    ...
    self::KEY_NOT_EMPTY   => 'Based on your answer above, this field is required',
  );
  ...
  public function doValid($value, $context = null) {
    ...
    if (is_array($context[$contextKey])) {
 	...
 	if (!is_null($testValue)) {
 		// hack added to deal with editor added returns in fields
      if ($testValue == ($parentField) && !empty($value) && '<br />'!=$value && '<br _moz_editor_bogus_node="TRUE" />'!=trim($value)) {
 		$this->_error(self::KEY_NOT_EMPTY);
        return false;
      }
    }
