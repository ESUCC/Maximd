 ...
 * @author   Jeremy Kendall 
 */
class My_Validate_BooleanNotEmptyIfContains extends Zend_Validate_Abstract {
				...
				if (! is_null ( $testValue )) {
					if (is_array($testValue)) {
						foreach($testValue as $tval) {
							if ($tval == ($parentValue) && empty ( $value ) && false !== $value) {
								$this->_error ( self::KEY_IS_EMPTY );
								return false;
							}
						}
					} else {
//						Zend_debug::dump('Parent:'.$parentValue.'/TestValue:'.$testValue.'/Value:'.$value, "multiple parent/single test - ".$contextKey);
						if ($testValue == ($parentValue) && empty ( $value ) && false !== $value) {
							$this->_error ( self::KEY_IS_EMPTY );
							return false;
						}
					}
				} else {
//					Zend_debug::dump('Parent:'.$parentValue.'/TestValue:'.$testValue.'/Value:'.$value, "multiple parent/test is null - ".$contextKey);
					if (! empty ( $parentValue ) && empty ( $value ) && false !== $value) {
						$this->_error ( self::KEY_IS_EMPTY );
						return false;
					}
				}
...
//					Zend_debug::dump('Parent:'.$parentValue.'/TestValue:'.$testValue.'/Value:'.$value, "single parent/single test - ".$contextKey);
					if ($testValue == ($parentValue) && empty ( $value ) && false !== $value) {
						$this->_error ( self::KEY_IS_EMPTY );
						return false;
					}
...
//				Zend_debug::dump('Parent:'.$parentValue.'/TestValue:'.$testValue.'/Value:'.$value, "single parent/test is null - ".$contextKey);
				if (! empty ( $parentValue ) && empty ( $value ) && false !== $value) {
