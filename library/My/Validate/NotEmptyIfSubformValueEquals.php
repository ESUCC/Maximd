 ...
 * @author   Jesse LaVere
 */
class My_Validate_NotEmptyIfSubformValueEquals extends Zend_Validate_Abstract {
...
//  	Zend_debug::dump('validationFieldValue:'.$validationFieldValue);
  ...
  public function doValid($validationFieldValue, $rowDataArray = null) {
    ...
    $validationFieldValue      = $validationFieldValue; // value of the field being validated
    $this->_setValue($validationFieldValue);

//    Zend_debug::dump($subformPrefix, 'subformPrefix');
//    Zend_debug::dump($subFormFieldName, 'subFormFieldName');
//    Zend_debug::dump($valueThree, 'valueThree');
//    Zend_debug::dump($validationFieldValue, 'validationFieldValue');
//    Zend_debug::dump($rowDataArray, 'rowDataArray');
//    Zend_debug::dump($rowDataArray[$subformPrefix]);
...
// 	    	Zend_debug::dump($row, 'row');
	...
	//	    Zend_debug::dump('$subformEleVal: '.$subformEleVal);
		    if (!is_null($subformEleVal)) {
	//	    	echo "here1";
		    	if ($subformEleVal == 1 && is_null($validationFieldValue) ) { //
	//	    		echo "here2";
		      		// error when value is not a boolean option
