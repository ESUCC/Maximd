 ...
 * @author   Jeremy Kendall 
 */
class My_Validate_EditorEmptyNotEmpty extends Zend_Validate_Abstract {
  ...
  protected $_messageTemplates = array(
    ...
    self::KEY_IS_EMPTY   => 'Based on your answer above, this field is required',
  		...
  		self::FIELD_FULL => 'Field cannot have a value',
  );
  ...
  public function doValid($value, $context = null) {
    ...
    if (!is_null($testValue)) {
    	// the field being tested is not empty
    	if ($testValue == ($parentField) && $fieldConsideredEmpty) {
    		// if match condition is true and field is empty - error
        	$this->_error(self::FIELD_EMPTY);
        	return false;
      	} elseif($testValue != ($parentField) && !$fieldConsideredEmpty) {
      		// if match condition is false and field is NOT empty
      		$this->_error(self::FIELD_FULL);
      		return false;
      	}
      	    
    } else {
    	// the field being tested is empty
    	// do not error.
    }
    return true;
  }
