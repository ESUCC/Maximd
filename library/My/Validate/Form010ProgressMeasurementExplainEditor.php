 ...
 * @author   Jeremy Kendall 
 */
class My_Validate_Form010ProgressMeasurementExplainEditor extends Zend_Validate_Abstract {
  ...
  protected $_messageTemplates = array(
	...
	self::FIELD_NOT_EMPTY => 'Field must NOT have a value', 
  );
  ...
  public function doValid($value, $context = null) {
    ...
    if($testValue == $parentFieldValue) {
        $fieldShouldBeEmpty = false;
    } else {
        $fieldShouldBeEmpty = true;
    }
    
    $fieldConsideredEmpty = false;
    if('' === $value || '<br _moz_editor_bogus_node="TRUE" />' === $value || '<br />'=== $value) {
    	$fieldConsideredEmpty = true;
    }
    
    if (!$fieldShouldBeEmpty && $fieldConsideredEmpty) {
        $this->_error(self::FIELD_EMPTY);
        return false;
    }
    if ($fieldShouldBeEmpty && !$fieldConsideredEmpty) {
        $this->_error(self::FIELD_NOT_EMPTY);
        return false;
    }
