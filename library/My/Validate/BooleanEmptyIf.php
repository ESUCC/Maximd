 ...
 * @author   Jeremy Kendall 
 */
class My_Validate_BooleanEmptyIf extends Zend_Validate_Abstract {
  ...
  public function doValid($value, $context = null) {
    ...
    $contextKey = $this->getContextKey();
//    $value      = $value;			// value of the field being validated
    ...
    if (!is_null($testValue)) {
    	if ($testValue == ($parentField) && is_null($value)) {
        	$this->_error(self::KEY_IS_EMPTY);
        	return false;
      	}
    } else {
      	if (!empty($parentField) && is_null($value)) {
        	$this->_error(self::KEY_IS_EMPTY);
        	return false;
      	}
    }
