 ...
 * @author   Jeremy Kendall 
 */
class My_Validate_NotEmptyIfAndEmptyIf extends Zend_Validate_Abstract {
  ...
  protected $_messageTemplates = array(
    ...
    self::FIELD_NOT_VALIDA => ':  If "Verification Decision" is "A", then value is required.  If "Verification Decision" is "B", value must be blank.',
    self::FIELD_NOT_VALIDB => ':  If "Verification Decision" is "B", then value is required.  If "Verification Decision" is "A", value must be blank.',
  );
   ...
   */
  public function __construct($contextKey, $testValue = false, $testValue2 = false) {
    $this->setTestValue($testValue);
    $this->setTestValue2($testValue2);
...
//    Zend_debug::dump(is_null($testValue));
    ...
    if (($parentField) == $testValue && strlen($value) > 0)
    {
        return true;
    }
    elseif (($parentField) == $testValue2 && empty($value))
    {
        return true;
    }
    else
    {
        if ($parentField == 'A')
            $this->_error(self::FIELD_NOT_VALIDA);
        else 
            $this->_error(self::FIELD_NOT_VALIDB);
...
//    		Zend_debug::dump('here');
   ...
   * @return string
   */
  protected function getTestValue2() {
  ...
  /**
   * @param string $testValue2
   */
  protected function setTestValue2($testValue2) {
    $this->_testValue2 = $testValue2;
  }
}
