    ...
    private function _createTables($schema, $tables, $master, &$diff, &$summary)
    {
    	if (count((array) $tables) > 0) {
	    	foreach ($tables as $table) {
	    		...
	    		if(isset($master['tables'][$table]['constraints'])) {
		    		foreach ((array) $master['tables'][$table]['constraints'] as $constraint => $constraintInfo) {
		    			switch ($constraintInfo['type']) {
		    				...
		    				// removed by jl because it was not using the primary key	
//							case 'PRIMARY KEY':
//		    					$constraintSrc = $constraintInfo['src'];
//		    					$__columns = array();
...
//		    					break;
	    		...
	    		foreach($_columns as $col) {
	    		    $fieldAndTypeWords = explode(' ', $col);
	    		    $field = array_shift($fieldAndTypeWords);
	    		    $type = implode(' ', $fieldAndTypeWords);
	    		    if('CONSTRAINT' != $field) {
    	    		    $columns .= "\"$field\" $type";
    	    		} else {
    	    		    $columns .= $col;
    	    		}
	    		    if($i != count($_columns)) $columns .= ",\n";
	    		    $i++;
	    		}
                
	    		$buffer = "\nCREATE TABLE {$schema}.{$table}(\n {$columns}\n)";
	    		if(null != $tablespace) $buffer.= "\nTABLESPACE {$tablespace}";
	    		$buffer.= ";";
	    		$buffer.= "\nALTER TABLE {$schema}.{$table} OWNER TO \"{$owner}\";";
	    		if(isset($master['tables'][$table]['grants'])) {
    ...
    private function _createSequence($schema, $sequence, $master, &$diff, &$summary)
    {
		...
		if(isset($master['sequences'][$sequence]['grants'])) {
    ...
    private function _addColumns($table, $columns, $master, &$diff, &$summary, $schema)
    {
    	if (count((array) $columns) > 0) {
	    	foreach ($columns as $column) {
	    		
				$columnType = $master['tables'][$table]['columns'][$column]['type'];
...
//	    		$summary['column']['create'][] = "{$schema}.{$view}.{$column}";
    ...
    private function _deleteColumns($table, $columns, $master, &$diff, &$summary, $schema)
    {
    	if (count((array) $columns) > 0) {
	    	foreach ($columns as $column) {
//	    		$diff[] = "delete column {$column} to table {$table}";
	    		$diff[] = "alter table {$table} drop column {$column}";
...
//    			$buffer[] = str_repeat("-", strlen($title));
    			...
    			$buffer[] = "Already synced: " . $row['db']->dbHost() . " :: " . $row['db']->dbName();
    		}
            ...
            if (md5(serialize($master)) == md5(serialize($slave))) {
                ...
                foreach ($masterTables as $table) {
                	...
                	if (count($newColumns) > 0) {
                		$this->_addColumns($table, $newColumns, $master, $diff, $summary, $schema);
                	}
                	
                	$deletedColumns = array_diff($slaveColumns, $masterColumns);
                	$this->_deleteColumns($table, $deletedColumns, $master, $diff, $summary, $schema);
         
                	foreach ($masterColumns as $column) {
                		// check modifications (different between $master and $slave)
                		// check differences in type
                		$masterType = $master['tables'][$table]['columns'][$column]['type'];
                		if(isset($slave['tables'][$table]['columns'][$column])) {
