...
$localConfig = new Zend_Config_Ini(APPLICATION_PATH . '/../scripts/cron/BEN/config.ini', APPLICATION_ENV);
echo "Configs loaded.\n";
...
PpnHelperBEN::setMissingIdStudentLocals($localConfig->idCounty,$localConfig->idDistrict);
$students = PpnHelperBEN::getStudentsForArchiving();
$filenames = array();
...
foreach ($students as $student) {
    ...
    $form = PpnHelperBEN::mostRecentFinalForm($student['id_student'], '004');
    if(!is_null($form)) {
        if(isset($student['unique_id_state'])) {
            $fileName = $student['unique_id_state'] . '_' . date('Ymd', strtotime($form['date_conference']));
            unset($filenames[$student['unique_id_state'] . '_' . date('Ymd', strtotime($form['date_conference'])) . '.pdf']);


            $sessUser = new Zend_Session_Namespace('user');
            $archiveData = PpnHelperBEN::archiveFormToPdf(
                'Model_Form004',
                '004',
                $sessUser,
                $form['id_form_004'],
