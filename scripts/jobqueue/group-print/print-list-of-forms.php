...
//    Zend_Debug::dump($config->DOC_ROOT);
    ...
    $oldSiteClient = new Zend_Http_Client('https://iepdev.nebraskacloud.org/logon.php?option=1', $httpParams);
    ...
    if($dom->query('#ferpa')->count()>=1) {
        // login failed
        Zend_Debug::dump('login failed on iepdev.nebraskacloud.org');
    }
...
//        Zend_Debug::dump($response, 'new');
        ...
        // old site
        $url = 'https://iepdev.nebraskacloud.org/form_print.php?form=form_'.$formNumber.'&document='.$document;
        if($summary) {
            $url .= '&summary=1';
        }
