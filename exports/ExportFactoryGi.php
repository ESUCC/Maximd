    ...
    /* print_r($table); // certainly did not print out the table name
      ...
      */        
        ...
        if (isset($this->exportConfig->limitToActive) && $this->exportConfig->limitToActive) { 
         $select->where('(status = ?','Active')->orWhere('sesis_exit_date > ?)',$this->exportConfig->SchoolYrStart);
          //$select->where('status = ?','Active');
        }
        /*
        SELECT "iep_student".* FROM "iep_student" WHERE ((id_county = '40' and id_district = '0002') or 
                                                         (id_county = '40' and id_district = '0003') or 
                                                         (id_county = '61' and id_district = '0004') or 
                                                         (id_county = '47' and id_district = '0100') or 
                                                         (id_county = '40' and id_district = '0082') or 
                                                         (id_county = '61' and id_district = '0049') or 
                                                         (id_county = '47' and id_district = '0001') or
                                                         (id_county = '40' and id_district = '0083') ) 
                                                         AND 
                                                         (status = 'Active') OR (sesis_exit_date > '2015-08-20')
            ...
            foreach ($configLine as $ffName => $fieldFormOrFunction) {
                if (substr_count($fieldFormOrFunction, '-') > 0) {
                    if (substr_count($fieldFormOrFunction, '-') == 1) {
                        list($formNumber, $draftOrFinal) = explode('-', $fieldFormOrFunction);//print_r($fieldFormOrFunction);echo "==\n";
                    } elseif (substr_count($fieldFormOrFunction, '-') == 2) {
                        list($formNumber, $draftOrFinal, $subFormName) = explode('-', $fieldFormOrFunction);//print_r($fieldFormOrFunction);echo "--\n";
                    }
...
//                    Zend_Debug::dump('form is null:'.$formNumber );
                    ...
                    if ('field' == $fieldFormOrFunction) {
                        ...
                        if (!method_exists($this, $ffName)) {
                            ...
                            $retArr[] = $this->sanitizeOutput($this->$ffName($student)); // print_r($retArr);
                        }
                    }
                }
            }
