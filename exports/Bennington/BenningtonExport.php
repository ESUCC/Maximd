...
class BenningtonExport extends ExportFactory {
    ...
    public function __construct() {
        ...
        $exportConfig = new Zend_Config_Ini('Bennington/export.ini', APPLICATION_ENV);
    ...
    public function getIepDataCard($student){
       ...
       $sql1="select id_student,status,date_conference,service_ot,service_pt,service_slt from iep_form_023 where id_student='".$student->id_student."' and status='Final' order by date_conference desc ";
    //   $this->writevar1($sql1,'this is the sql1 statement'); 
    ...
    public function primaryService2($student,$iep) {
    ...
    public function booleanOcupationalTherapy($student){
        
        $iep=$this->lastIep($student);
        $iepDataCard=$this->getIepDataCard($student);
      
      if($iepDataCard !=null && $iepDataCard['date_conference']>=$iep->date_conference && $iepDataCard['status']=='Final') 
      {
         
               $bool=$iepDataCard['service_ot'];
      ...
      { 
        $service=$this->primaryService2($student,$iep);
        if ($service=='Occupational Therapy Services'){
            $bool='true';
        }
        else{
            $bool='false';
        }
        $relatedServ= $this->relatedServices($student);
        $services=explode(':',$relatedServ);
        foreach($services as $serv){
            if ($serv=='Occupational Therapy Services') {
                $bool='true';
            }
        }
      }
    ...
    public function booleanPhysicalTherapy($student){
    ...
    public function booleanSpeechLanguage($student){
        ...
        $service=$this->primaryService2($student,$iep);
        if ($service=='Speech-language therapy'){
            $bool='true';
        }
        else{
            $bool='false';
        ...
        $services=explode(':',$relatedServ);
        foreach($services as $serv){
            if ($serv=='Speech-language therapy') {
                $bool='true';
            }
        }
