...
$exportLine[24]=$this->nssrsChangeValue($exportLine[22]);
 ...
 //  Mike  on 1-11-2016 did this so that the values would show up in powerschool as a 0 instead of a blank
      ...
      if($exportLine[11]==''||$exportLine[10]==NULL){
          $exportLine[11]=0;
      }
...
//          $t=preg_replace('/\s+/', '', $t);
          ...
          $t=$exportLine[25];
//          $t=preg_replace('/\s+/', '', $t);
          $exportLine[25]=substr($t,0,3995);
          $exportLine[25]=$exportLine[25];
          if($exportLine[25]==NULL) $exportLine[25]='No Qualified  FBA is Available';
           ...
           //             print_r($retArr); just sanitizes the output to an array of the iep_student
                       ...
                       /*   
                         if(!is_null($retArr[9])) {
                           ...
                           $newDateString = date_format(date_create_from_format('Y-m-d', $retArr[9]), 'd-m-Y');
    ...
    public function mostRecentFinalForm($student, $formNo) {
     ...
     //  $this->writevar1($modelName,'this is the model name');
        ...
        try {
           ...
           if ($modelName=='Model_Table_Form019' && $form != NULL) {
            //   $this->writevar1($form['date_notice'],'this is the form date');
               $now =time();
               $theDate=strtotime($form['date_notice']);
               $differ=$now-$theDate;
               $differ2=$differ/(60*60*24);
            //   $this->writevar1($differ2,'this is the difference from 1-1-2016');
               
               if(count($form)&& $modelName!='Model_Table_Form019')  {
                   $this->$localVar = $form;
                  $this->writevar1($differ2,'we got into the not table 0019');
                   return $form;
               }
               
               if(count($form)&& $modelName=='Model_Table_Form019') {
                   $this->writevar1($form['date_notice'],'this is before it checks with the 365 days');
                   if($differ2<=365.0) {
                       $this->writevar1($form['date_notice'],'this is if it is less than 365 days and form 19');
                       return $form;
                   }
                   else {
                       $this->writevar1($student->id_student,'this iss the student');
                       $this->writevar1($form['date_notice'],' this gets a null return');
                       return NULL;
                   }
               }
